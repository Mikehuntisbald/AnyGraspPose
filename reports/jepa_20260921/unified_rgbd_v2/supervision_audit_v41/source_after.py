...
def real_case(factory,seed,out,index,quality_gap=None):
    ...
    row=dict(seed=seed,stream=ep.stream,frame=frame,object_id=stream['object_id'],diameter_m=s.diameter,
             ...
             pre_quality_real_pixels=int(original_real.sum()),removed_real_pixels=int((original_real&~real).sum()),
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--config',required=True);p.add_argument('--out',type=Path,required=True);p.add_argument('--cases',type=int,default=16);p.add_argument('--quality-gap',type=float);a=p.parse_args()
    ...
    for i in range(a.cases):
        row=real_case(factory,40000000+i,a.out,i,a.quality_gap);result['cases'].append(row)
        (a.out/'audit.json').write_text(json.dumps(result,indent=2));print(json.dumps(row),flush=True)
    result.update(completed=True,hard_checks_passed=True,quality_gap_m=a.quality_gap,scope='bounded train frames; annotation accuracy not assumed; raster edge outliers separately reported',
