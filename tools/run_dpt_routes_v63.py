...
root=Path('/mnt/why/dexycb_lip/unified_jepa_20260921/dpt_routes_v63')
root.mkdir(exist_ok=False)
runtime=Path('/tmp/dexycb_dpt_routes_v63_r0')
...
try:
 for rank in range(8):
  ...
  children.append(subprocess.Popen([sys.executable,'tools/probe_dpt_routes_v63.py','--out',str(root/f'rank{rank}'),'--rank',str(rank),'--records','8'],cwd=runtime,env=env,stdout=log,stderr=subprocess.STDOUT))
