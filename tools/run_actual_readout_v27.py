"""One bounded actual-latent readout probe, three same-seed feature sources."""
import os,sys,subprocess,json,time,signal,hashlib,tarfile
from pathlib import Path

def main():
    exe=Path(__file__).resolve().parents[1];root=Path('/mnt/why/dexycb_lip/unified_jepa_20260921/actual_readout_v27');root.mkdir(exist_ok=False);children=[]
    ...
    try:
        ...
        run('tests',[['-m','pytest','-q','tests/jepa/test_cached_actual_readout.py']])
        run('cache',[['tools/cache_actual_readout_v27.py','--config','configs/jepa/amp_preview_v25_fixed.yaml','--checkpoint','/mnt/why/dexycb_lip/unified_jepa_20260921/serial_completion_v21/runs/seed42/last.pt','--out',str(root/'cache'/f'rank{rank}'),'--rank',str(rank),'--world','8'] for rank in range(8)])
        run('fit',[['tools/fit_actual_readout_v27.py','--cache',str(root/'cache'),'--out',str(root/arm),'--source',arm,'--steps','1000'] for arm in ('observed_visible','decoded','patch')])
        result={arm:json.loads((root/arm/'receipt.json').read_text()) for arm in ('observed_visible','decoded','patch')}
