...
def main():
    ...
    for row,(name,d) in enumerate(data):
        ...
        images=[np.moveaxis(d['occluded_rgb'][0],0,-1),xyz,
                np.where(mask,xyz_error,np.nan),np.where(mask,depth_error,np.nan),d['gate'][0,0]]
        for col,image in enumerate(images):
            options=dict(cmap='magma',vmin=0,vmax=40) if col in (2,3) else (dict(cmap='viridis',vmin=0,vmax=1) if col==4 else {})
            artist=axes[row,col].imshow(np.clip(image,0,1) if col<2 else image,**options)
            axes[row,col].set_xticks([]);axes[row,col].set_yticks([])
            if row==0:axes[row,col].set_title(['Occluded real RGB','Predicted canonical XYZ','XYZ error (mm)','Depth error (mm)','Transport gate'][col])
            if col==0:axes[row,col].set_ylabel(name)
            if row==2 and col>=2:fig.colorbar(artist,ax=axes[:,col],shrink=.65)
    fig.suptitle('Fixed training-partition holdout: identical input and full eligible real/proxy regions')
