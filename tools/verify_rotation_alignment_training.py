...
def main():
    ...
    for device,precision,count in [('cpu','fp32',8),('cuda','fp32',16),('cuda','bf16',16)]:
        ...
        with torch.no_grad():
            ...
            if c.get('rotation_alignment',False):
                parent_config=dict(c,rotation_alignment=False)
                parent=make_model(parent_config).to(device).eval();load_init(e['parent'],parent,audit,parent_config)
                parent_out=StreamTrainingModule(parent,dict(probe,rotation_alignment=False),renderer)([s],True)['predictions'][0]
                assert torch.equal(parent_out,training), 'Zero-start parent real trajectory mismatch'
                del parent
    ...
    activity=[]
    hook=model.rotation_alignment.register_forward_hook(lambda module,args,out:activity.append(float(out.detach().float().norm(dim=-1).mean())))
    ...
    for step in range(3):
        ...
        if step>0:assert model.rotation_alignment.cad[0].weight.grad.norm()>0, 'CAD encoder receives no gradient'
    ...
    hook.remove();assert max(activity)>0
    ...
    receipt=dict(passed=True,source_sha256=source_hash(),parent_sha256=e['parent_sha256'],
        ...
        alignment_activity=activity,cad_gradient_nonzero_after_first_update=True,zero_start_parent_real_trajectory_bitwise_equal=True,rgb_unchanged=True,all_trainable_gradients_finite=True,changed_tensors=changed,
