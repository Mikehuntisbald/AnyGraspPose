...
def main():
 ...
 assert exact(initial['model'],source['model']) and initial['step']==source['step']==40400
 def states(record):
 ...
 new=set(begin)-set(old);assert not new and exact(initial['optimizer'],source['optimizer'])
 assert all(torch.equal(v,last['model'][n]) for n,v in source['model'].items() if n.startswith(('core.feature_mid.','core.feature_last.','core.log_error.')))
 ...
 assert int(last['model']['ema_updates'])==int(initial['model']['ema_updates'])+last['step']-40400
 ranks=[]
 for rank in range(8):
  ...
  assert row['recovery_metrics']['pose']>0 and row['dino_feature_supervision'] is False and row['ema_teacher_forward'] is False;ranks.append(row['step'])
 assert last['sampler_position']==32*last['step'] and len(last['rng'])==8
 result=dict(passed=True,step=last['step'],checkpoint_sha256=sha(r/'last.pt'),initial_model_exact=True,inherited_moments_exact=True,new_pose_parameters=len(new),entire_initial_optimizer_exact=True,unused_feature_heads_unchanged=True,pose_and_jepa_updated=True,history_unchanged=True,all_rank_steps=ranks,ema_counter_continuous=True)
