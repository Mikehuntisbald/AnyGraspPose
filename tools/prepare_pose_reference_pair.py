"""Prepare a matched parent continuation versus zero-start retained-pose feedback."""
import argparse
import json
...
def main():
    ...
    e=dict(parent=str(parent_path.resolve()),parent_sha256=a.expected_parent_sha,source_sha256=source_hash(),split_hash=audit['split_hash'],mesh_hash=audit['mesh_hash'],
        ...
        seed=42,steps=1000,reader_arm='pose_reference',arms={},training_manifest=str(manifest.resolve()),training_manifest_sha256=a.expected_manifest_sha,
        reference_residence_clock='First update or external correction timestamp; not physical pose-measurement time.',temporal_occlusion_probability=.5,reference_evaluation=str(reference.resolve()),quality_head_enabled=False,foundationpose_calls=0,test_access=False,
        ...
        trained_modules='Existing small modules in both arms; reference arm adds a zero-initialized observation/state-conditioned feedback head towards the retained provided pose. Parent backbone, temporal encoder and original pose head remain frozen.',
        action_semantics='Two signed tanh coefficients interpolate/extrapolate parent rotation-vector and center/d actions towards the retained supplied pose. Zero equals parent. One equals reference component. Not probabilities or SE(3) interpolation; no GT motion/visibility gate. Reference captured at first update, never refreshed by predictions; external correction may refresh it.')
    for name in ('control','pose_reference'):
        folder=a.out/name;folder.mkdir();cfg=dict(c,preflight_receipt=str((folder/'approval.json').resolve()))
        if name=='pose_reference':cfg['architecture_id']='stream_rk_pose_reference'
    ...
    old_names=set(e['arms']['control']['trainable']);new_names=set(e['arms']['pose_reference']['trainable'])
    assert old_names<=new_names and all(n.startswith('pose_reference_feedback.') for n in new_names-old_names)
