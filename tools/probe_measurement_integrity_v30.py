"""Teacher-isolated causal diagnostics; oracle arms are never deployment candidates."""
...
def main():
    ...
    with torch.no_grad():
        while any(counts[k]<needed[k] for k in counts):
            ...
            for case,base in cases:
                ...
                teacher_render=factory.renderer(e.cad['appearance'],gt[0],scene.k_crop,224)
                gtdepth=teacher_render['depth'].reshape(1,1,224,224)
                gap=(obs.measured_depth_m-gtdepth).abs()
                geometric_visible=shown&(gtdepth>0)&(gap<=max(.01,.05*d))
                ...
                for arm,conf in [('original',None),('dense',probability),('oracle_selection',probability*shown),('oracle_selection_xyz',probability*shown),('oracle_depth_selection',probability*geometric_visible),('oracle_depth_selection_xyz',probability*geometric_visible),('oracle_measured_only',probability*geometric_visible)]:
                    packet=pack_completion(**arguments,measurement_probability=conf)
                    # Oracle intervention AFTER the complete student forward.
                    # Backproject actual sensor depth with GT only for evaluation.
                    actual_camera=obs.crop_rays*obs.measured_depth_m/obs.diameter[:,None,None,None]-gt[:,:3,3,None,None]/obs.diameter[:,None,None,None]
                    true_xyz=torch.einsum('bji,bjhw->bihw',gt[:,:3,:3],actual_camera)
                    real=shown&(obs.measured_depth_m>0)&torch.isfinite(obs.measured_depth_m)
                    error=(packet['xyz']-true_xyz).norm(dim=1,keepdim=True)*d*1000
                    measured_true=(packet['measured_weight']>0)&real
                    newly_true=measured_true&~(output['evidence_logits'].sigmoid().reshape(1,1,16,16).repeat_interleave(14,-2).repeat_interleave(14,-1)>=.7)
                    if arm in ('oracle_selection_xyz','oracle_depth_selection_xyz','oracle_measured_only'):packet=dict(packet,xyz=torch.where(measured_true,true_xyz,packet['xyz']))
                    if arm=='oracle_measured_only':packet=dict(packet,weight=packet['measured_weight'],completed_weight=torch.zeros_like(packet['weight']))
                    with torch.autocast('cuda',dtype=torch.bfloat16):
                    ...
                    row['arms'][arm]=dict(rgb_visible_measured_pixels=int(measured_true.sum()),depth_inconsistent_pixels=int((measured_true&~geometric_visible).sum()),real_xyz_error_sum_mm=float((error*measured_true).sum()),real_xyz_count=int(measured_true.sum()),new_xyz_error_sum_mm=float((error*newly_true).sum()),new_xyz_count=int(newly_true.sum()),rotation=float(rotation[0]),center_d=float((pose[:,:3,3]-gt[:,:3,3]).norm()/d),
    ...
    receipt=dict(completed=True,groups=counts,rows=len(records),checkpoint_sha256=sha(a.checkpoint),result_sha256=sha(root/'records.json'),selector_sha256=sha(a.selector),legacy_forward_exact=True,teacher_encoder_forward=False,teacher_geometry_input=True,student_groundtruth_input=False,oracle_readout_groundtruth_input=True,optimizer_updates=0,training_split_only=True,paired_rgbd_exact=True,scope='Controlled training-split readout probe; estimated-pose augmentation uses GT only to construct supervised training cases')
