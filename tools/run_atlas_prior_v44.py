"""Matched100-update control and final coordinate-prior supervision."""
import json,signal,subprocess,sys,time
from pathlib import Path


def main():
    root=Path('/mnt/why/dexycb_lip/unified_jepa_20260921/atlas_prior_v44');root.mkdir(exist_ok=False)
    ...
    try:
        for arm in ('control','supervised'):
            with (root/f'{arm}.launch.log').open('w') as log:
                child=subprocess.Popen([sys.executable,'tools/run_geometry_transport_warmup.py','--config',f'configs/jepa/atlas_prior_v44_{arm}.yaml','--root',str(root/arm),'--steps','100','--clean-probe'],cwd=exe,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
