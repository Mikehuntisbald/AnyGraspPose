...
def main():
    ...
    def train(folder,steps,single=False,resume=False,preflight=True):
        commands=[]
        for i,name in enumerate(('control','alignment')):
        ...
        return commands
    try:
        run('tests',[('cpu','',[sys.executable,'-m','pytest','-c',str(root/'pyproject.toml'),'tests/test_intraframe_alignment.py','tests/test_real_initialization.py','tests/test_stream_startup_training.py','tests/test_stream_batch_features.py','tests/test_stream_state.py','tests/test_stream_causality.py','tests/test_stream_migration.py','tests/test_stream_sampling_contract.py','tests/test_val_non_gt_protocol.py','tests/test_stream_review_regressions.py','-m','not cuda','-q','--junitxml='+str(r/'tests.xml')],r/'tests.log')])
        ...
        run('real_interface_and_gradient',[('both','0',[sys.executable,'tools/verify_rotation_alignment_training.py','--arm','alignment','--experiment',str(r)],r/'equivalence.log')])
