"""Two200-update matched trials: legacy vs clean CAD surface identity."""
...
def main():
    ...
    root=Path('/mnt/why/dexycb_lip/unified_jepa_20260921/geometry_surface_identity_v38');root.mkdir(exist_ok=False)
    child=None
    def status(stage,**kw):
    ...
    try:
        for arm in ('raw','canonical'):
            with (root/f'{arm}.launch.log').open('w') as log:
                child=subprocess.Popen([sys.executable,'tools/run_geometry_transport_warmup.py','--config',f'configs/jepa/geometry_surface_identity_v38_{arm}.yaml','--root',str(root/arm),'--steps','200'],cwd=exe,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
