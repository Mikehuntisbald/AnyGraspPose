    ...
    with torch.no_grad():
        while any(counts[k]<needed[k] for k in counts):
            ...
            for case,base in cases:
                ...
                for arm,conf in [('original',None),('dense',probability),('oracle_selection',probability*shown),('oracle_selection_xyz',probability*shown)]:
                    ...
                    if arm=='oracle_selection_xyz':packet=dict(packet,xyz=torch.where(measured_true,true_xyz,packet['xyz']))
                    with torch.autocast('cuda',dtype=torch.bfloat16):
                    ...
                    row['arms'][arm]=dict(real_xyz_error_sum_mm=float((error*measured_true).sum()),real_xyz_count=int(measured_true.sum()),new_xyz_error_sum_mm=float((error*newly_true).sum()),new_xyz_count=int(newly_true.sum()),rotation=float(rotation[0]),center_d=float((pose[:,:3,3]-gt[:,:3,3]).norm()/d),
