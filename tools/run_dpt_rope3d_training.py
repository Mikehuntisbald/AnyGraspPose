"""Bounded 1000-update DPT/RoPE/normal experiment; serial exclusive GPU jobs."""
...
def main():
    ...
    state=dict(completed=False,pid=os.getpid(),source_step=start,target_step=end,mode='dpt_rope3d_derived_normals_jepa_only')
    child=None
    def status(value,**kw):
    ...
    env=dict(os.environ,PYTHONPATH=str(root/'src'),CUDA_VISIBLE_DEVICES='0,1,2,3,4,5,6,7',OMP_NUM_THREADS='2',OPENBLAS_NUM_THREADS='2',TORCHINDUCTOR_COMPILE_THREADS='1',CUBLAS_WORKSPACE_CONFIG=':4096:8',TORCHINDUCTOR_CACHE_DIR='/tmp/dexycb_dpt_rope_v17_inductor',TRITON_CACHE_DIR='/tmp/dexycb_dpt_rope_v17_triton')
    def run(name,args):
        nonlocal child
    ...
    try:
        ...
        evaluate('source_mlp','configs/jepa/dpt_rope3d_v17_baseline.yaml',c['reconstruction_only']['source_checkpoint'])
        run('report',[sys.executable,'tools/report_dpt_rope3d.py','--root',str(root)])
