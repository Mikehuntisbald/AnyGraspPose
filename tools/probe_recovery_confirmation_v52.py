"""Prospective new-frame JEPA recovery confirmation; no pose or training."""
...
def main():
    ...
    root=base/'recovery_balance_v52/confirmation';root.mkdir(exist_ok=False)
    ...
    try:
        ...
        for arm in ('source','control','balanced'):
            ...
            for rank in range(8):
                ...
                cmd=[sys.executable,'tools/probe_geometry_transport.py','--config',f"configs/jepa/recovery_balance_v52_{'control' if arm=='source' else arm}.yaml",
                     '--checkpoint',str(base/'recovery_balance_v52'/('control' if arm=='source' else arm)/'runs/seed42'/('initial.pt' if arm=='source' else 'last.pt')),'--out',str(root/arm/f'rank{rank}'),
                     '--rank',str(rank),'--records','8','--seed-start','54000000']
