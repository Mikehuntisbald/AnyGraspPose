...
def main():
    ...
    summaries = {step: json.loads((root/f'diagnostics/step{step}/summary.json').read_text())
                 for step in (12000, 15000, 20000, 25000)}
    source = summaries[12000]
    ...
    for region, metric, label, factor, unit in selections:
        ...
        row['final_minus_initial'] = row['25000']-row['12000']; rows.append(row)
    ...
    lines = ['# V15 extension: completed13000 additional JEPA-only updates', '',
             ...
             '| Metric |12000|15000|20000|25000|final-initial|', '|---|---:|---:|---:|---:|---:|']
    lines += [f"|{r['label']} ({r['unit']})|{r['12000']:.4f}|{r['15000']:.4f}|{r['20000']:.4f}|{r['25000']:.4f}|{r['final_minus_initial']:+.4f}|" for r in rows]
    (directory/'REPORT.md').write_text('\n'.join(lines)+'\n')
    (directory/'receipt.json').write_text(json.dumps(dict(completed=True, updates=13000,
        fixed_feature_teacher=source['fixed_feature_teacher'], endpoint_checkpoint=summaries[25000]['checkpoint_sha256']), indent=2))
