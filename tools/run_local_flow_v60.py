...
def main():
    exe=Path(__file__).resolve().parents[1];root=Path('/mnt/why/dexycb_lip/unified_jepa_20260921/local_flow_v60')
    ...
    try:
        ...
        for split,records in (('train',256),('holdout',32)):
            run('collect_'+split,[['tools/collect_local_flow_v60.py','--out',str(root/'cache'/split/f'rank{i}'),
