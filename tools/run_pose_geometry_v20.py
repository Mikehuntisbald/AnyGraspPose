"""Bounded 40400->45400 joint training with serialized native-val and recovery evaluation."""
...
def main():
 ...
 assert c['training']['max_steps']-c['migration']['source_step']==5000
 ...
 state=dict(completed=False,pid=os.getpid(),source_step=40400,target_step=45400,mode='pose_geometry_only');children=[]
 ...
 env=dict(os.environ,PYTHONPATH=str(exe/'src'),CUDA_VISIBLE_DEVICES='0,1,2,3,4,5,6,7',OMP_NUM_THREADS='2',OPENBLAS_NUM_THREADS='2',TORCHINDUCTOR_COMPILE_THREADS='1',CUBLAS_WORKSPACE_CONFIG=':4096:8',TORCHINDUCTOR_CACHE_DIR='/tmp/dexycb_pose_geometry_v20_inductor',TRITON_CACHE_DIR='/tmp/dexycb_staged_rope_v18_triton')
 ...
 def audit(name):run(name,[['tools/audit_pose_geometry.py','--config',a.config,'--out',str(root/(name+'.json'))]])
 ...
 try:
  ...
  train(40402);train(40403);audit('startup_audit')
  native('source40400',out/'initial.pt')
  for step in (41400,45400):
