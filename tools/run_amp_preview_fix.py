"""Paired 200-update AMP-preview fix, queued after the active V24 controller."""
import fcntl,json,os,signal,subprocess,sys,time,tarfile,hashlib
from pathlib import Path
import yaml

def main():
    exe=Path(__file__).resolve().parents[1];config='configs/jepa/amp_preview_v25_fixed.yaml';c=yaml.safe_load((exe/config).read_text())
    ...
    try:
        parent_pid=2018156
        parent_status=Path('/mnt/why/dexycb_lip/unified_jepa_20260921/shape_conditioned_v24/grounded_recovery/controller/status.json')
        ...
        while proc.exists():
            ...
            status('waiting_for_v24',parent_pid=parent_pid,parent_start_ticks=identity);time.sleep(5)
        parent=json.loads(parent_status.read_text())
        if not parent.get('completed'):raise RuntimeError('V24 did not complete; do not overlap or silently replace it')
        ...
        run('cuda_gradient_proof',[['tools/probe_amp_preview_cuda.py','--config',config,'--checkpoint',c['geometry_priority']['source_checkpoint'],
            '--legacy-source','/tmp/dexycb_shape_conditioned_v24_train/src/lip/unified/cad_surface.py','--out',str(root/'cuda_gradient_proof.json')]])
        train(1001,True)
