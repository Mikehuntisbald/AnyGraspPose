"""Dense measurement cache from raw sensor, estimated CAD, and frozen CAD descriptors."""
    ...
    with torch.no_grad():
        while any(counts[k]<needed[k] for k in counts):
            ...
            for case,base in cases:
                ...
                row=dict(rgb=cpu(obs.packet.rgb_crop),reference=cpu(normalize_rgb(current.render['rgb'][None])),geometry=cpu(obs.geometry_image),
                    cad=cpu(obs.geo),available=obs.cad_valid.any(-1).cpu(),bounds=scene.bounds.cpu(),target=shown.cpu(),depth_valid=depth_valid.cpu(),
                    original_probability=cpu(output['evidence_logits'].sigmoid().reshape(1,1,16,16).repeat_interleave(14,-2).repeat_interleave(14,-1)),
                    physical=physical,stream=e.stream,split=kind,seed=seed,case=case,heavy=heavy,added_visible_fraction=fraction)
                records.append(row)
            counts[kind]+=1
