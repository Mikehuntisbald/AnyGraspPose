"""Bounded visibility information probe, three equal-capacity heads."""
import os,sys,subprocess,json,time,signal,hashlib,tarfile
from pathlib import Path

def main():
    exe=Path(__file__).resolve().parents[1];root=Path('/mnt/why/dexycb_lip/unified_jepa_20260921/visibility_probe_v28');root.mkdir(exist_ok=False);children=[]
    ...
    try:
        ...
        run('cache',[['tools/cache_visibility_probe_v28.py','--config','configs/jepa/amp_preview_v25_fixed.yaml','--checkpoint','/mnt/why/dexycb_lip/unified_jepa_20260921/serial_completion_v21/runs/seed42/last.pt','--out',str(root/'cache'/f'rank{rank}'),'--rank',str(rank),'--world','8'] for rank in range(8)])
        run('fit',[['tools/fit_visibility_probe_v28.py','--cache',str(root/'cache'),'--out',str(root/arm),'--source',arm,'--steps','1000'] for arm in ('raw','fused','patch')])
        result={arm:json.loads((root/arm/'receipt.json').read_text()) for arm in ('raw','fused','patch')}
