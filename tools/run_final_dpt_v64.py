...
def main():
    ...
    root = Path('/mnt/why/dexycb_lip/unified_jepa_20260921/final_dpt_v64')
    root.mkdir(exist_ok=False)
    config = 'configs/jepa/final_dpt_v64.yaml'
    out = root/'runs/seed42'
    ...
    try:
        ...
        run('tests', [['-m','pytest','-q','tests/jepa/test_flow_reconstruction.py','tests/jepa/test_local_flow.py','tests/jepa/test_dpt_final_route.py',
        ...
        # Inactive routing reproduces the historical matched control's first2 updates.
        run('control_replay2',[['-m','torch.distributed.run','--standalone','--nproc_per_node=8',
            'tools/fp_worker.py','tools/train_geometry_transport.py','--config','configs/jepa/final_dpt_v64_control_replay.yaml','--stop-at','2']])
        compared=[]
        for rank in range(8):
            previous=Path('/mnt/why/dexycb_lip/unified_jepa_20260921/flow_joint_v62_r1/strong/seed42')/f'rank{rank}.jsonl'
            current=root/'control_replay/seed42'/f'rank{rank}.jsonl'
            old=[json.loads(x) for x in previous.read_text().splitlines()][:2]
            new=[json.loads(x) for x in current.read_text().splitlines()]
            assert len(new)==2
            for x,y in zip(old,new):
                for k in ('step','loss','grad_norm','lr','metrics','windows'):
                    if x[k]!=y[k]:raise RuntimeError(f'Historical control replay differs: rank{rank} step{x["step"]} {k}')
            compared.append(rank)
        (root/'control_replay_exact.json').write_text(json.dumps(dict(ranks=compared,steps=2,loss_grad_metrics_data_exact=True)))
        for step in (2,200):
            command=['-m','torch.distributed.run','--standalone','--nproc_per_node=8',
                'tools/fp_worker.py','tools/train_geometry_transport.py','--config',config,'--stop-at',str(step)]
            if step==200:command+=['--resume',str(out/'last.pt')]
            run(f'train{step}',[command])
        for angle,records in ((0,4),(10,8),(60,4)):
            probe(f'final_{angle}',out/'last.pt',cfg=config,angle=angle,records=records)
        status('complete', completed=True, updates=200,historical_control_replay_verified=True, default_model_changed=False)
