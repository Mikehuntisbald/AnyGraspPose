...
def main():
 p=argparse.ArgumentParser();p.add_argument('--config',required=True);p.add_argument('--checkpoint',required=True);p.add_argument('--out',required=True);p.add_argument('--rank',type=int,required=True);p.add_argument('--world',type=int,default=8);p.add_argument('--smoke',action='store_true');a=p.parse_args()
 ...
 m=build_model(c);checkpoint=Path(a.checkpoint);saved=torch.load(checkpoint,map_location='cpu',weights_only=False);checkpoint_step=saved['step'];load_core(m,saved['model']);del saved
  ...
  for physical,sid in selected:
   ...
   for f in range(first,max(frames)+1):
    ...
    for tag,base,previous in bases:
     ...
     row=dict(physical_sequence=physical,stream_id=sid,frame_index=f,object_id=s['object_id'],symmetry=symmetry,visibility=refs['lip'][sid,f]['visibility'],base_kind=tag,metrics=stats,deltas=deltas,
 ...
 receipt=dict(completed=True,training=False,optimizer_updates=0,official_test_access=False,rank=a.rank,world=a.world,smoke=a.smoke,rows=rows,selection=selection,checkpoint_step=checkpoint_step,entrypoint_sha256=sha(__file__),checkpoint_sha256=m.weights_version,lip_checkpoint_sha256=sha(LIP/'selected.pt'),state_digests_before=before,state_digests_after=after,
  maximum_crop_difference=max(alignment),old_lip_replay_max_abs=max(replay),seconds=time.monotonic()-started,frames_sha256=sha(outdir/'frames.jsonl'),scope='Fixed40 physical sequences; up to3 selected frames each; conditional perturbation diagnostics. GT bases/surface/support are oracles; point scores use cached mesh points. Controlled pose cases have no motion-state hint; LIP history is causal original-only replay.')
