...
def main():
 ...
 tb=build_teachers(*teacher_args,real_geometry_max_radius_d=1.,fast=True,batch_render=True)
 target_errors={}
 ...
 for encoder in (reference.encoder,model.ema_teacher):
  with torch.no_grad(),torch.autocast('cuda',dtype=torch.bfloat16):
   ...
   del graph
 arms=[('baseline',{}),('graphs_fast',dict(reference_dino_graph=True,teacher_dino_graph=True,fast_geometry=True)),
       ('batched_teacher',dict(reference_dino_graph=True,teacher_dino_graph=True,fast_geometry=True,batch_teacher_render=True)),
       ('frames16',dict(reference_dino_graph=True,teacher_dino_graph=True,fast_geometry=True,batch_teacher_render=True,frame_batch=16)),
       ('frames32',dict(reference_dino_graph=True,teacher_dino_graph=True,fast_geometry=True,batch_teacher_render=True,frame_batch=32))]
