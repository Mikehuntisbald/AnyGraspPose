...
def main():
    exe=Path(__file__).resolve().parents[1];root=Path('/mnt/why/dexycb_lip/unified_jepa_20260921/measurement_integrity_v30');root.mkdir(exist_ok=False);children=[]
    ...
    try:
        ...
        run('cache',[['tools/probe_measurement_integrity_v30.py','--config','configs/jepa/amp_preview_v25_fixed.yaml','--checkpoint','/mnt/why/dexycb_lip/unified_jepa_20260921/serial_completion_v21/runs/seed42/last.pt','--out',str(root/'cache'/f'rank{rank}'),'--selector','/mnt/why/dexycb_lip/unified_jepa_20260921/dense_measurement_v29/fit/measurement.pt','--rank',str(rank),'--world','8'] for rank in range(8)])
        rows=[]
        ...
        for name,part in [('nonsym_rotation',[r for r in rows if not r['symmetric'] and r['case'] in ('positive','negative')]),('nonsym_augmented_rotation',[r for r in rows if not r['symmetric'] and r['heavy'] and r['case'] in ('positive','negative')]),('zero',[r for r in rows if r['case']=='zero']),('mixed',[r for r in rows if r['case']=='mixed'])]:
            result[name]=dict(cases=len(part),arms={arm:{k:sum(r['arms'][arm][k] for r in part)/len(part) for k in ('rotation','center_d','true_measurement_fraction_of_all_weight','false_measurement_fraction_of_all_weight','real_xyz_error_sum_mm','real_xyz_count','new_xyz_error_sum_mm','new_xyz_count','rgb_visible_measured_pixels','depth_inconsistent_pixels')} for arm in ('original','dense','oracle_selection','oracle_selection_xyz','oracle_depth_selection','oracle_depth_selection_xyz','oracle_measured_only')})
