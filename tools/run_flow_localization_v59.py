...
root=Path('/mnt/why/dexycb_lip/unified_jepa_20260921/flow_localization_v59')
root.mkdir(exist_ok=False)
runtime=Path('/tmp/dexycb_flow_localization_v59_r0')
...
try:
 for rank in range(8):
  ...
  children.append(subprocess.Popen([sys.executable,'tools/probe_flow_localization_v59.py','--out',str(root/f'rank{rank}'),'--rank',str(rank),'--records','32'],cwd=runtime,env=env,stdout=log,stderr=subprocess.STDOUT))
 ...
 (root/'status.json').write_text(json.dumps(dict(stage='complete',frames=256)))
