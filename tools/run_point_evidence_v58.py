...
def main():
    exe=Path(__file__).resolve().parents[1];root=Path('/mnt/why/dexycb_lip/unified_jepa_20260921/point_evidence_v58')
    ...
    try:
        ...
        run('tests',[['-m','pytest','-q','tests/jepa/test_point_evidence.py','tests/jepa/test_flow_reconstruction.py','tests/jepa/test_flow_surface_audit.py']])
        for split,records in (('train',256),('holdout',16)):
            run('collect_'+split,[['tools/collect_point_evidence_v58.py','--out',str(root/'cache'/split/f'rank{i}'),
                '--rank',str(i),'--split',split,'--records',str(records)] for i in range(8)],True)
        run('train2',[['tools/train_point_evidence_v58.py','--root',str(root),'--stop-at','2']])
        run('train1000',[['tools/train_point_evidence_v58.py','--root',str(root),'--resume','--stop-at','1000']])
        for angle,records in ((10,8),(60,4)):
            for arm in ('baseline','control','observed'):
                ...
                run(f'probe_{arm}_{angle}',[['tools/probe_geometry_transport.py','--config',config,'--checkpoint',checkpoint,
                    ...
                    '--seed-start','58050000','--rotation-degrees',str(angle),'--flow-surface-audit'] for i in range(8)],True)
