"""Parent preservation for a registered retained-pose feedback tracker."""
import argparse
import json
...
def main():
    ...
    for precision in ('fp32','bf16'):
        ...
        assert torch.equal(predictions['control'],predictions['pose_reference'])
    ...
    (a.experiment/'equivalence.json').write_text(json.dumps(dict(passed=True,parent_sha256=e['parent_sha256'],source_sha256=source_hash(),
        scope='Registered pose-reference architecture on two identical actual training fragments with configured augmentation; zero-residual function preservation',results=results),indent=2))
