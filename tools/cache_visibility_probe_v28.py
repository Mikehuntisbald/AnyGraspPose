"""Visibility calibration cache from actual inputs; labels never enter student forward."""
...
def main():
    ...
    def capture(name):
        ...
        return hook
    hooks=[m.visibility.register_forward_pre_hook(capture('feature_raw')),m.core.blocks[0].register_forward_pre_hook(capture('feature_fused'))]
    draw=0
    with torch.no_grad():
        while any(counts[k]<needed[k] for k in counts):
            ...
            for case,base in cases:
                ...
                row.update(feature_patch=cpu(output['patch_latent']),
                    target=cpu(F.avg_pool2d(shown.float(),14,14).flatten(1)),valid=patch_valid.cpu(),
                    depth_count=cpu(F.avg_pool2d(depth_valid.float(),14,14).flatten(1)*196),
                    visible_depth_count=cpu(F.avg_pool2d((depth_valid&shown).float(),14,14).flatten(1)*196),
                    original_probability=cpu(output['evidence_logits'].sigmoid()),physical=physical,stream=e.stream,
                    split=kind,seed=seed,case=case,heavy=heavy,added_visible_fraction=fraction)
                records.append(row)
            counts[kind]+=1
