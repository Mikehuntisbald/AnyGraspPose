"""Matched softsign rotation-anchor retraining from the same fixed adaptive parent."""
import argparse
import json
...
def main():
    ...
    e=dict(parent=str(parent_path.resolve()),parent_sha256=a.expected_parent_sha,source_sha256=source_hash(),
        ...
        reader_arm='smooth_rotation',seed=42,steps=1000,arms={},training_manifest=str(manifest.resolve()),training_manifest_sha256=a.expected_manifest_sha,
        ...
        selection='Require real non-GT val initialization and matched full val before selecting a candidate; do not choose using noisy-GT val alone. No official test in this training controller.')
    for name in ('control','smooth_rotation'):
        ...
        if name=='smooth_rotation':cfg['architecture_id']='stream_rk_rotation_anchor_smooth'
    ...
    old_names=set(e['arms']['control']['trainable']);new_names=set(e['arms']['smooth_rotation']['trainable'])
