"""Paired controlled geometry outcomes; equal eligible frame means."""
...
def main():
    ...
    for ax,(region,metric) in zip(axes.flat,[(r,m) for r in ('real','proxy') for m in ('canonical_xyz_mm','depth_mm')]):
        for i,arm in enumerate(('baseline','control','strong')):
            values=[data[f'{arm}_{angle}'][f'heavy_{region}'][metric] for angle in (0,10,60)]
