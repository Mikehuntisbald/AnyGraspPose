...
def main():
    ...
    root=Path(args.root)
    tags=tuple(f'{arm}_{angle}' for angle in (10,60) for arm in ('baseline','control','extra'))
    data={}
    for tag in tags:
        rows=[]
        for rank in range(8):
    ...
    result={};lines=['# V60 local flow matched readout: full forward results','',
