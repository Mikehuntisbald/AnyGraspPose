...
def main():
    ...
    p.add_argument('--intervention',choices=('learned','zero'),required=True);p.add_argument('--startup-iterations',type=int,choices=(1,2),default=1);p.add_argument('--rank',type=int,default=0);p.add_argument('--world',type=int,default=1)
    ...
    assert c.get('rotation_alignment') and a.startup_iterations==1
    def tensor_hash():
        h=hashlib.sha256()
        for name,value in sorted(model.state_dict().items()):h.update(name.encode());h.update(value.detach().cpu().contiguous().numpy().tobytes())
        return h.hexdigest()
    versions=model.parameter_versions();before_hash=sha(a.checkpoint);model_hash=tensor_hash();capture={}
    def capture_rotation(module,args,out):
        capture['raw_rotation']=out.detach()
        return torch.zeros_like(out) if a.intervention=='zero' else None
    hook=model.rotation_alignment.register_forward_hook(capture_rotation)
    original_forward=model.forward_encoded
    def capture_parent(*args,**kwargs):
        result,cache=original_forward(*args,**kwargs);capture['parent_pose']=result['pose_centered'].detach()
        return result,cache
    model.forward_encoded=capture_parent
    receipt=dict(model_tensors_sha256=model_hash,alignment_intervention=a.intervention,weights_unchanged=True,diagnostic_only=True,startup_iterations=a.startup_iterations,extra_inner_attempts=0,inner_fallbacks=0,completed=False,split='val',architecture_id=c['architecture_id'],source_sha256=source_hash(),
        ...
        for stream in streams:
            ...
            for frame in range(count):
                ...
                if external is not None and frame>=external['frame_index']:
                    ...
                    if not is_initial and status=='ok':
                        row['unapplied_rotation_vec']=capture['raw_rotation'][0].float().cpu().tolist()
                        row['pose_before_alignment']=capture['parent_pose'][0].float().cpu().tolist()
    ...
    hook.remove();assert model.parameter_versions()==versions and sha(a.checkpoint)==before_hash and tensor_hash()==model_hash
