"""Generate the factual delivery report from completed local receipts."""
from pathlib import Path
import json,hashlib
p=Path('.')
read=lambda path:json.loads((p/path).read_text())
r=read('runs/subset_preflight_receipt.json');o=r['overfit'];g=read('runs/subset_geometry/geometry.json');cl=read('runs/eval_mixed_subset_closed/metrics.json');one=read('runs/eval_mixed_subset_onestep/metrics.json');probe=read('configs/resolved_8gpu.probe.json')
import numpy as np
median_depth=np.median([s['depth_abs_median_m'] for s in g['samples']])*1000
batch32=[x for x in probe['results'] if x['batch']==32 and x['rollout']==4][0]
text=f'''# 实施状态：DexYCB-LIP v1
...
| 几何审计 | 32/32 样本有足够可见像素；每样本可见区域深度绝对误差中位数再取中位数为 {median_depth:.3f} mm；最大 crop 投影误差 {max(s['crop_projection_max'] for s in g['samples']):.8f} px | `runs/subset_geometry/geometry.json`, `*_overlay.jpg`, `*_depth.png` |
| 真实32-clips过拟合 | 按训练混合采样固定32个不重复clips、关闭噪声/增强/dropout，500 optimizer steps。loss {o['initial']['loss']:.8f} → {o['final']['loss']:.8f}；ADD {o['final']['zero_motion']['add_m']*1000:.3f} → {o['final']['learned']['add_m']*1000:.3f} mm；center {o['final']['zero_motion']['center_mm']:.3f} → {o['final']['learned']['center_mm']:.3f} mm | `runs/overfit32_mixed_subset/report.json`, `manifest.json`, `loss.jsonl` |
| 单卡真实 batch probe | 4/8/16/32 均完成单步与U=4前后向及optimizer step。选32；U=4 peak allocated {batch32['peak_allocated']/2**30:.3f} GiB，reserved {batch32['peak_reserved']/2**30:.3f} GiB，低于实测初始可用显存80% | `configs/resolved_8gpu.probe.json`, `runs/probe_real_subset.log` |
| 8卡真实DDP | 每卡32、accum=1、有效batch256，50个optimizer steps，全部U=4。8个rank均无非有限值；每rank最大allocated峰值的最大值 {r['maximum_gpu_allocated_bytes']/2**30:.3f} GiB | `runs/ddp_real_subset/rank0.jsonl` 至 `rank7.jsonl`, `runs/ddp_real_subset_launcher.log` |
...
- 平均 center error：{cl['micro']['center_mm']['mean']:.3f} mm。
- 平均 rotation error：{cl['micro']['rotation_deg']['mean']:.3f} deg。
- ADD recall@0.1d：{cl['micro']['add_01']['mean']*100:.3f}%；ADD-S recall@0.1d：{cl['micro']['adds_01']['mean']*100:.3f}%。
- lost fraction：{cl['micro']['lost']['mean']*100:.3f}%（连续5帧ADD-S>0.1d触发；所有帧仍在分母）。
- 同步测得batch=1端到端均值 {cl['latency_seconds']['end_to_end']['mean']*1000:.3f} ms，p95 {cl['latency_seconds']['end_to_end']['p95']*1000:.3f} ms；含磁盘读取/解码、crop/render/network，排除GT指标与可视化，不含FP。此延迟不能弥补跟踪漂移。

统一32个noisy-history单步诊断的平均ADD：zero-motion {one['zero-motion']['micro']['add_m']['mean']*1000:.3f} mm、constant-velocity {one['constant-velocity']['micro']['add_m']['mean']*1000:.3f} mm、learned {one['learned']['micro']['add_m']['mean']*1000:.3f} mm。这是训练子集诊断，不是held-out泛化证据，亦不能证明latent学会交互。
...
'''
(p/'runs/STATUS.md').write_text(text)
print('wrote runs/STATUS.md from completed receipts')
