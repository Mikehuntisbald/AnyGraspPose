"""Bounded 10000-update measured-first/recovered-later RoPE experiment; serial exclusive GPU jobs."""
...
def main():
    ...
    assert end-start==10000 and c['budget']['updates']==10000
    ...
    try:
        ...
        for offset in (1000,5000,10000):
            ...
            shutil.copy2(out/'last.pt',root/('milestones/update'+str(offset)+'.pt'))
        ...
        destination=root/'diagnostics/source_v18';destination.mkdir(exist_ok=True)
        ...
        run('rope_switch10000',[sys.executable,'tools/evaluate_recovery_focus.py','--config',a.config,'--checkpoint',str(out/'last.pt'),'--out',str(root/'diagnostics/rope_switch10000'),'--rope-ablation'])
