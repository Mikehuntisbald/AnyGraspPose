"""No-GT intervention: cap aggregate completion mass when measured evidence exists."""
    ...
    with torch.no_grad():
        while any(counts[k]<needed[k] for k in counts):
            ...
            for case,base in cases:
                ...
                for arm,conf in [('original',None),('dense',probability),('original_balanced',None),('dense_balanced',probability)]:
                    packet=pack_completion(**arguments,measurement_probability=conf,completion_mass_ratio=1. if arm.endswith('_balanced') else None)
                    with torch.autocast('cuda',dtype=torch.bfloat16):
