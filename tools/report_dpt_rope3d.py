...
def main():
    ...
    p.add_argument('--source',default='source_mlp');p.add_argument('--title',default='DPT + 3D RoPE + XYZ-derived normal supervision');a=p.parse_args()
    names=(a.source,'update500','update1000')
    ...
    rows=[];text=['# '+a.title,'',
        'Same fixed40 protocol and fixed step11000 EMA feature teacher. Pose is frozen; this is recovery evaluation. Architecture/objective changes are combined, so differences cannot identify their individual effects.','',
        '| Case | Target / metric | '+a.source+' | +500 | +1000 |','|---|---|---:|---:|---:|']
    ...
    for ax,(region,metric) in zip(axes.flat,selections[:6]):
        ...
        ax.bar([a.source,'+500','+1000'],[row[n] for n in names],color=['#999999','#407ba7','#198b78'])
