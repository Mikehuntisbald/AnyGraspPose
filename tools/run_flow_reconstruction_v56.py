...
def main():
    ...
    root = Path('/mnt/why/dexycb_lip/unified_jepa_20260921/flow_reconstruction_v56')
    root.mkdir(exist_ok=False)
    config = 'configs/jepa/flow_reconstruction_v56.yaml'
    ...
    def probe(tag, checkpoint, cfg=config, ablation='on', angle=10, records=8):
        run(tag, [['tools/probe_geometry_transport.py','--config',cfg,'--checkpoint',str(checkpoint),
            ...
            '--seed-start','56050000','--flow-ablation',ablation,'--rotation-degrees',str(angle)] for i in range(8)], True)
    try:
        ...
        for step in (2,200):
            ...
            if step == 2:
                probe('initial', out/'initial.pt')
                probe('baseline_v53', '/mnt/why/dexycb_lip/unified_jepa_20260921/recovery_formal_v53/runs/seed42/last.pt',
                      cfg='configs/jepa/recovery_formal_v53.yaml')
        for mode in ('on','no_feedback','no_transport'):
            probe('final_'+mode, out/'last.pt', ablation=mode)
        probe('large_baseline', '/mnt/why/dexycb_lip/unified_jepa_20260921/recovery_formal_v53/runs/seed42/last.pt',
              cfg='configs/jepa/recovery_formal_v53.yaml', angle=60, records=4)
        probe('large_final', out/'last.pt', angle=60, records=4)
        run('report', [['tools/report_flow_reconstruction_v56.py','--root',str(root)]])
        status('complete', completed=True, updates=200, default_model_changed=False)
