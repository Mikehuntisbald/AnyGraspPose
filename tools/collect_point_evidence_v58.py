"""Frozen V56 features; training and physical-holdout ownership are disjoint."""
...
from lip.unified.point_evidence import point_evidence_targets
...
def main():
    ...
    m.requires_grad_(False).eval();m.fast_geometry=m.vector_geometry=True;m.flow_reconstruction.capture_point_evidence=True
    f=Factory(c,m,make_store(c,m));a.out.mkdir(exist_ok=False,parents=True)
    features=[];labels=[];groups=[];legacy=[];records=[];draw=0;begun=time.monotonic()
    with torch.no_grad():
        while len(records)<a.records:
            seed=(58000000 if a.split=='train' else 58050000)+a.rank+8*draw;draw+=1
            ...
            value,_=point_evidence_targets(out,target,obs,visible,scene.k_crop[None])
            known=torch.isfinite(value).any(-1)
            x=out['flow_rounds'][-1]['point_evidence_features'][known].float().cpu()
            y=value[known].cpu();features.append(x);labels.append(y)
            groups.append(torch.full((len(x),),item,dtype=torch.long))
            legacy.append(out['flow_rounds'][-1]['legacy_visible_logits'][known].float().cpu())
            row=dict(seed=seed,stream=e.stream,heavy=heavy,base_kind=base_kind,points=len(x),
                visible_positive=int((y[:,0]==1).sum()),quality_positive=int((y[:,1]==1).sum()))
            records.append(row)
            if len(records)%32==0:print(json.dumps(dict(records=len(records),seconds=time.monotonic()-begun)),flush=True)
    torch.save(dict(features=torch.cat(features),labels=torch.cat(labels),groups=torch.cat(groups),legacy_logits=torch.cat(legacy),records=records),a.out/'features.pt')
    (a.out/'receipt.json').write_text(json.dumps(dict(completed=True,source_sha256=DIGEST,features_sha256=sha(a.out/'features.pt'),
        ...
        quality_definition='Known front point; original currently-visible measurement at predicted UV; endpoint error <=3px; positive finite depth within30mm of GT CAD depth. Original reconstruction labels unchanged.',
