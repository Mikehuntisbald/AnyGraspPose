...
def main():
    ...
    if a.resume:
        ...
        del saved
    fixed=json.loads(Path(a.fixed_manifest).read_text()) if a.fixed_manifest else None
    ...
    with (output/f'rank{rank}.jsonl').open('a') as log:
        while step<stop:
            started=time.perf_counter();optimizer.zero_grad(set_to_none=True);logs=[];loss_log=[];count=0
