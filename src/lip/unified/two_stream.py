...
def encode_two_stream(model,scenes,masks=None,cad_enabled=None,frame_id=0,occlusions=None):
    ...
    atlas=encode_surface(model,scenes,depth,cad_enabled,full=True) if hasattr(model,'cad_atlas_decoder') else None
    ...
    return Observation(packet,mid[:b],last[:b],mid[b:],last[b:],pixel_valid&cad_enabled[:,None],
        ...
        geometry_image=torch.cat(geometry),cad_surface_features=local_cad[0],cad_surface_geometry=local_cad[1],cad_surface_valid=local_cad[2],crop_rays=rays,rope_depth_stats=depth_stats,measured_depth_m=depth,cad_atlas=atlas)
