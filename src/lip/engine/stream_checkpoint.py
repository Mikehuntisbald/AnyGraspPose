...
def migrate(model,checkpoint,allow_untrained_single=False):
    ...
    if old_arch not in ('legacy_v1','stream_single'):raise ValueError('Use --resume within an existing architecture')
    ...
    for name,tensor in target.items():
        ...
        if reset:entry['reason']='state semantics changed or new immutable-position/time-bias module'
    ...
    skipped={name:dict(status='skipped',reason='obsolete v1 state/time/readout representation or non-target module') for name in src if name not in used}
    ...
    result=dict(parent=parent,target_architecture=model.architecture_id,cache_contract=CACHE_CONTRACT,
...
def save_init(path,model,config,migration):
    ...
    torch.save(dict(model=model.state_dict(),architecture_id=model.architecture_id,cache_contract=CACHE_CONTRACT,
...
def load_init(path,model,audit,config):
    ...
    for key,value in [('architecture_id',model.architecture_id),('cache_contract',CACHE_CONTRACT),('split_hash',audit['split_hash']),('mesh_hash',audit['mesh_hash'])]:
...
def save(path,model,optimizer,scheduler,step,config,audit,sampler_position,parent):
    ...
    obj=dict(model=model.state_dict(),optimizer=optimizer.state_dict(),scheduler=scheduler.state_dict(),
        architecture_id=model.architecture_id,cache_contract=CACHE_CONTRACT,new_stage_step=step,global_step=step,
...
def resume(path,model,optimizer,scheduler,audit,config,rank=0,world_size=1):
    obj=torch.load(path,map_location='cpu',weights_only=False)
    expected=dict(architecture_id=model.architecture_id,cache_contract=CACHE_CONTRACT,
