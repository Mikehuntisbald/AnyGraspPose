# DexYCB-LIP v1：2026-09-10 远端已有数据续做
...
| 几何检查 | 32/32有效样本，覆盖20物体；可见深度每样本中位误差再取中位数 **6.595 mm**；最大crop投影误差 **0.00009155 px** | `geometry/geometry.json`、32组overlay/depth图 |
| 采样统计 | train **270,336**个候选clips；moving **185,528**，heavy occlusion **7,579**，unknown visibility **2,666**；桶可重叠。采用规定的50/25/25混合 | `sampling_summary.log` |
| 真实32-clips过拟合 | 固定混合采样，涉及15物体、29段录制；500步，关闭噪声/增强/dropout；loss **0.244994 → 0.129346**，ADD **22.658 → 10.373 mm** | `overfit32/report.json`、`manifest.json`、`loss.jsonl` |
...
| 8卡DDP | 50 optimizer steps，全部U=4，无hang/NaN，各rank样本与step一致；最大allocated峰值 **18.406 GiB/卡** | `ddp/rank0.jsonl` 至 `rank7.jsonl`、`ddp_launcher.log` |
| 恢复 | 从step50恢复到53，8个rank的global step/scheduler step/采样位置一致；没有声称逐位确定性 | `ddp_resume.log`、`receipt.json` |
| 验证入口 | 官方val固定跨20物体：单步32 clips三方比较；20个完整stream连续闭环共 **1448帧**，未GT重置或丢掉lost帧 | `val_onestep/`、`val_closed/` |
...
|---|---:|---:|---:|
| Zero-motion | 27.467 | 11.230 | 29.306 |
| Constant-velocity | 13.868 | 9.536 | 16.483 |
| Learned | 26.888 | 11.528 | 28.948 |

Learned尚未超过恒速基线。连续闭环只在每stream首帧使用GT初始化一次：

- 平均center误差 **396.757 mm**，rotation误差 **53.427 deg**。
- micro ADD recall@0.1d **11.326%**；micro ADD-S recall@0.1d **15.953%**。
- macro-over-object ADD-S recall@0.1d **16.034%**。
- lost fraction **78.522%**，采用任务书连续5帧ADD-S>0.1d标准，全部1448帧保留。
- batch1同步端到端均值 **25.513 ms**，p95 **27.638 ms**。包括读取/解码/crop/render/network，排除GT metrics和可视化，没有FP。
...
4. 没有本次`best.pt`：没有用quick val选择best。当前checkpoint位于远端：`runs/available_20260910/overfit32/last.pt`（500步）与`runs/available_20260910/ddp/last.pt`（53步）。
